...
Quelques rôles "globaux" permettent d'accéder à toutes les données {stackstate-product-name} - dans n'importe lequel des groupes observés.  Ces rôles sont destinés à la configuration du système et à la résolution des problèmes au niveau du système.  Pour les utilisateurs ayant l'un de ces rôles, il n'est pas nécessaire de configurer des xref:scoped[rôles étendus].
